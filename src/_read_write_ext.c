...
static PyObject *
_read_table_ext(PyObject *self, PyObject *args) {

    char *filename;
    PyArrayObject *table_data; // This is the python array object we are going to pupulate
    int n_rows=0, n_cols=0; // number of rows and columns in table data file
    ...
    dims[0] = n_rows;
    dims[1] = n_cols;
    table_data = PyArray_SimpleNew(2, dims, NPY_DOUBLE);
    ...
    while ((read = getline(&row, &len, fp)) != -1) {
        ...
        if (is_data){
            ...
            while( (col = strsep(&row," ")) != NULL ){
                ...
                if (strings_starts_with(col, "\t")) 
                    continue;
                *(double *)PyArray_GETPTR2(table_data, n_rows, n_cols) = atof(col);
                n_cols++;
            }
            n_rows++;
        }
    ...
    return PyArray_Return(table_data);
...
MOD_INIT(_read_write_ext)
{
    PyObject *m;

    MOD_DEF(m, "_read_write_ext", "Routines for reading and writing data.\n", _read_ext_methods)
